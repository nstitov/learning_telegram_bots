...
from bot.cbdata import (
    ...
    IgnoreCallbackFactory,
    NewGameCallbackFactory,
...
from bot.minesweeper.game import (
    all_flags_match_bombs,
    all_free_cells_are_open,
    gather_open_cells,
    get_fake_newgame_data,
    get_real_game_data,
    make_text_table,
...
async def callbak_newgame(
    call: CallbackQuery, state: FSMContext, callback_data: NewGameCallbackFactory
):
    ...
    kb = make_keyboard_from_minefield(
        newgame_dict["game_data"]["cells"], game_id, ClickMode.CLICK
    )
...
async def callback_open_square(
    call: CallbackQuery,
    state: FSMContext,
    callback_data: ClickCallbackFactory,
    session: AsyncSession,
):
    """Called when player clicks a HIDDEN cell (without any flags ot numbers)."""
    ...
    # This cell contained a number
        ...
        # If cell is empty (0), open all adjucent squares
        ...
        # There are more flags than there should be
            ...
            with suppress(TelegramBadRequest):
                await call.message.edit_reply_markup(
                    reply_markup=make_keyboard_from_minefield(
                        cells,
                        game_id,
                        game_data["current_mode"],
                    )
                )
            await call.answer(
                show_alert=True,
                text="Looks like you've placed more flags than there are bombs on "
                "field. Please check them again.",
            )
            return
...
@router.callback_query(
    SwitchModeCallbackFactory.filter(), flags={"need_check_game": True}
)
async def switch_click_mode(
    call: CallbackQuery, state: FSMContext, callback_data: SwitchModeCallbackFactory
):
    ...
    if game_data["initial"] is True:
        await call.answer(
            show_alert=True, text="You can only place flags after first click!"
        )
        return

    game_data["current_mode"] = callback_data.new_mod
    await state.update_data(game_data=game_data)

    with suppress(TelegramBadRequest):
        await call.message.edit_reply_markup(
            reply_markup=make_keyboard_from_minefield(
                cells, game_id, game_data["current_mode"]
            )
        )
    await call.answer()
...
@router.callback_query(
    SwitchFlagCallbackFactory.filter(), flags={"need_check_game": True}
)
async def add_or_remove_flag(
    call: CallbackQuery,
    state: FSMContext,
        ...
        if untouched_cells_count(cells) == 0:
            if all_flags_match_bombs(cells):
                ...
                with suppress(TelegramBadRequest):
                    ...
                    await call.answer(
                        show_alert=True,
                        text="Look like you've placed more flags then there are bombs "
                        "on field. Please check them again.",
                    )
                return
        else:
            await state.update_data(game_data=game_data)
...
async def callback_ignore(call: CallbackQuery):
    """Calles when player clicks on an open number. In this case, we simply ignore this
    callback.
    """
