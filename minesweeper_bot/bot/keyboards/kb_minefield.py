...
def make_keyboard_from_minefield(
    cells: List[List], game_id: str, click_mode: int
        ...
        for cell in cells_row:
            ...
            if mask_value == CellMask.HIDDEN:
                ...
                if click_mode == ClickMode.CLICK:
                    btn.callback_data = ClickCallbackFactory(
                        game_id=game_id, x=x, y=y
                    ).pack()
                else:
                    btn.callback_data = SwitchFlagCallbackFactory(
                        game_id=game_id, action="add", x=x, y=y
                    ).pack()
                ...
                btn = InlineKeyboardButton(
                    ...
                    callback_data=SwitchFlagCallbackFactory(
                        game_id=game_id, action="remove", x=x, y=y
                    ).pack(),
                )
                ...
                if val == 0:
                    val = "⠀"  # Empty symbol
                btn = InlineKeyboardButton(
                    text=str(val), callback_data=IgnoreCallbackFactory(x=x, y=y).pack()
                )
            kb_row.append(btn)
        keyboard.row(*kb_row)
    ...
    if click_mode == ClickMode.FLAG:
        switch_mode_btn = InlineKeyboardButton(
            ...
            callback_data=SwitchModeCallbackFactory(
                game_id=game_id, new_mod=ClickMode.CLICK
            ).pack(),
        )
    else:
        switch_mode_btn = InlineKeyboardButton(
            ...
            callback_data=SwitchModeCallbackFactory(
                game_id=game_id, new_mod=ClickMode.FLAG
            ).pack(),
        )
