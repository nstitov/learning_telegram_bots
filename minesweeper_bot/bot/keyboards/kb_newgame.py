...
def make_newgame_keyboard() -> InlineKeyboardMarkup:
    available_options = [(5, 3), (6, 5), (7, 7)]  # (size, bombs)
        ...
        keyboard.row(
            InlineKeyboardButton(
                ...
                callback_data=NewGameCallbackFactory(
                    size=size, bombs=bombs, as_separate=False
                ).pack(),
            )
        )
...
def make_replay_keyboard(size: int, bombs: int) -> InlineKeyboardMarkup:
    ...
    keyboard.row(
        InlineKeyboardButton(
            text=f"New game {size}x{size} feild, {bombs} bombs",
            callback_data=NewGameCallbackFactory(
                size=size, bombs=bombs, as_separate=True
            ).pack(),
        )
    )
    keyboard.row(
        InlineKeyboardButton(text="New game (other)", callback_data="choose_newgame")
    )
    return keyboard.as_markup()
